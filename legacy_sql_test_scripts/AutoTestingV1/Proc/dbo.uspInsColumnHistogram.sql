...
BEGIN
	...
	BEGIN
		SET @sql = FORMATMESSAGE('INSERT INTO TestLog.dbo.ColumnHistogram (ColumnProfileID, ColumnValue, ValueCount, ColumnHistogramTypeID) VALUES (%i, ''%s'', %i, %i)',@pColumnProfileID, CAST(@columnValue AS varchar(200)), @valueCount, @pColumnHistogramTypeID)
		-- RAISERROR(@sql, 0, 1) WITH NOWAIT;
		EXEC(@sql);
		FETCH NEXT FROM valueCursor INTO @valueCount, @columnValue;
	END
