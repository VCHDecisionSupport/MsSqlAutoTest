...
--#region BR dbo.CubeSource
SET @UpdatedBy = 'VCH\GCrowell'
SET @CreatedBy ='VCH\GCrowell'
SET @IsActive = 1;
SET @IsLogged = 1;
SET @GUID = '7a808df8-77c7-11e6-b989-00232444dba2'
...
ON dbo.AssessmentHeaderFact.SourceAssessmentID=dbo.CubeSource.SourceAssessmentID
...
ON dbo.CaseNoteHeaderFact.SourceCaseNoteHeaderID=dbo.CubeSource.SourceCaseNoteHeaderID
	JOIN dbo.PersonFact
...
JOIN Dim.DischargeDisposition
ON Dim.DischargeDisposition.DischargeDispositionID=dbo.CubeSource.DischargeDispositionID
JOIN Dim.ReferralPriority
ON Dim.ReferralPriority.ReferralPriorityID=dbo.CubeSource.ReferralPriorityID
JOIN Dim.ReasonEndingService
ON Dim.ReasonEndingService.ReasonEndingServiceID=dbo.CubeSource.ReasonEndingServiceID
JOIN Dim.ReferralSourceLookup
ON Dim.ReferralSourceLookup.ReferralSourceLookupID=dbo.CubeSource.ReferralSourceLookupID
	JOIN Dim.ReferralSource
	ON Dim.ReferralSource.ReferralSourceID=Dim.ReferralSourceLookup.ReferralSourceID
JOIN Dim.ReferralSource
ON Dim.ReferralSource.ReferralSourceID=dbo.CubeSource.ReferralSourceID
JOIN Dim.AssessmentType
ON Dim.AssessmentType.AssessmentTypeID=dbo.CubeSource.AssessmentTypeID
END
'
EXEC [dbo].[SetBizRule] 
@pBRId=@BRID, 
@pShortNameOfTest='dbo.CubeSource', 
@pRuleDesc='', 
@pConditionSQL='', 
@pActionID=, 
